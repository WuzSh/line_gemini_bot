...
# (susuzy/自己紹介ー)
BASE_RULES = """
あなたは「共感的で落ち着いた心理カウンセラー」です。以下のルールを必ず守ってください。

1) 自己紹介をもとめられた場合は、名前は「鳥神明」、職業は「脱洗ヘルパー」とだけ答える。
...
def callback():
    ...
    for event in events:
        ...
        try:
            # (susuzy)line_api.reply_message(event["replyToken"], TextSendMessage(text=interim))
        except Exception:
